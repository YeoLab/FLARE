...
def get_number_edited_reads(row):
    ...
    return int(round(int(total_reads) * float(fraction)))
...
def combine_and_reformat(fwd, rev, output, force_rewrite=True):
    """
    Combines the "fwd" and "rev" edit calls into a single file, and
    reformats original SAILOR code such that the name column contains 
    the actual edit conversion total, as well as the total coverage at that position.
    Note: If both "fwd" and "rev" edit calls are 0, then this function will 
    create an empty file and return 0. 
    """
    if not os.path.exists(output) or force_rewrite:
        df = concat_bedfiles(fwd_file=fwd, rev_file=rev)
        if df.shape[0] == 0:  # if there are 0 edits (common for single cell), don't return error. Instead, return empty file
