...
def non_target_edit_mismatches(read_seq, md, edit_type, sense):
    """
    Given a read sequence, MD tag, target edit type (AG or CT, for example)
    and 'sense' (ie. for AG target edit type, look for AG if sense, look for TC if antisense), 
    return the number of non-target mismatches seen in the read.

    :param read_seq: string
    :param md: string
    :param sense: boolean
    :return num non-target edits: int
    """
    ...
    if mismatches:
        ...
        for mismatch in mismatches:
            ...
            read_allele = read_seq[read_pos].upper()
            if sense:
                if not (ref_allele == origin_target_base and read_allele == destination_target_base):
                    non_target_mm_counts += 1
            else:
                if not (ref_allele == COMPLEMENT.get(origin_target_base) and read_allele == COMPLEMENT.get(destination_target_base)):
                    non_target_mm_counts += 1
            read_pos += 1

    return non_target_mm_counts


def filter_reads(
        input_bam, output_bam, min_overhang, min_underhang,
        mm_tolerance, edit_type, reverse_stranded=True
):
    """
    # Step 3: filter reads
    ...
    * Removed 5d per discussions with the Hundley lab.
    ...
    :param mm_tolerance: int
        any more mismatches than this (aside from the expected edit) will filter the read.
    :param edit_type: str
        which is the expected edit type, for example CT or AG
    :return:
    """
    ...
    for read in i:
        try:
            ...
            #     threshold. If non_ag_mm_counts > mm_tolerance, toss the whole
            #     read. Otherwise, allow up to the maximum allowable non-AG
            #     mismatches before tossing. Default: 1 mm
            """

            if non_target_edit_mismatches(read_seq, mm, edit_type, sense) > mm_tolerance:
                flags['non_target_edits_exceeded'].append(read_name)
                flag = 7
...
''' % (program_shortdesc, str(__date__))
    ...
    try:
        ...
        parser.add_argument(
            "-mmt", "--mm_tolerance",
            dest="mm_tolerance",
            help="allow up to [n] other types of mutations other than the goal mutation in a read before tossing."
                 " Default: 1",
            required=False,
            type=int,
            default=1
