...
def pass_editing_site_phenotype(ref, alt, sense, edit_type):
    """
    Returns True if it looks like an editing site (for example, if edit_type is AG,
    we are looking for A-G on sense or T-C antisense).
    ...
    if ((ref == origin_target_base and alt.find(destination_target_base) == -1) or
        (ref == COMPLEMENT.get(origin_target_base) and alt.find(COMPLEMENT.get(destination_target_base)) == -1)):
        return False
    
    # sense variants must have origin target as ref, antisense must have complement of origin target as ref
    if (sense and ref != origin_target_base) or (not sense and ref != COMPLEMENT.get(origin_target_base)):
        return False
...
def vcf2eff(input_vcf, output_eff, min_coverage, edit_type, cov_metric='DP4',
            reverse_stranded=True, unstranded=False):
    """
        ...
        for line in f:
            ...
            if line.startswith('#'):  # is vcf header
                ...
                try:
                    ...
                    """
                    if not pass_editing_site_phenotype(ref, alt, sense, edit_type):
                        flag = 3
...
''' % (program_shortdesc, str(__date__))
    ...
    try:
        ...
        flags = vcf2eff(input_vcf, output_eff, min_coverage, edit_type, dp, is_reverse, unstranded)
        if save_filtered:
